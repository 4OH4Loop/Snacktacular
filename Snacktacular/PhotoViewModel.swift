...
class PhotoViewModel {
    ...
    static func saveImage(spot: Spot, photo: Photo, data: Data) async {
        ...
        do {
            ...
            // Now photo file is saved to Storage, save a Photo document to the spot.id's "photos" collection
            let db = Firestore.firestore()
            do {
                try db.collection("spots").document(id).collection("photos").document(photo.id ?? "n/a").setData(from: photo)
            } catch {
                print("😡 ERROR: Could not update data in spots/\(id)/photos/\(photoName). \(error.localizedDescription)")
            }
        } catch {
